...
| 系列 | 英文类型 | 中文类型 | 槽位 | 基础作用 |
| --- | --- | --- | ---: | --- |
| `sword` | Sword | 长剑 | 4 | 攻击 +8、防御 +1、最大生命 +4。 |
| `axe` | Axe | 战斧 | 4 | 攻击 +12、最大生命 +3、攻击速度 -8%。 |
| `bow` | Bow | 长弓 | 4 | 攻击 +6；随阶段提高攻击速度。 |
| `staff` | Staff | 法杖 | 4 | 攻击 +7。 |
| `helmet` | Helmet | 头盔 | 2 | 防御 +1、最大生命 +4。 |
| `chest` | Chestplate | 胸甲 | 3 | 防御 +3、最大生命 +10。 |
| `gloves` | Gloves | 手套 | 0 | 攻击 +2；随阶段提高攻击速度。 |
| `shoes` | Boots | 战靴 | 1 | 防御 +1、最大生命 +4；随阶段提高闪避。 |
| `cloak` | Cloak | 披风 | 5 | 防御 +1、最大生命 +5；随阶段提高闪避。 |
| `accessory` | Amulet | 护符 | 7 | 攻击 +1、最大生命 +5。 |
...
坐骑是 `mount` 分类的唯一装备实例，统一占用槽位 6。当前都只由抽奖获得，固定为完美品质；每只提供最终最大生命 +5%、最终攻击 +5%、移动速度 +25%。
